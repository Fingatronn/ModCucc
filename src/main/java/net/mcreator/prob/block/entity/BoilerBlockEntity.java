...
public class BoilerBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(3, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public BoilerBlockEntity(BlockPos position, BlockState state) {
		super(ProbModBlockEntities.BOILER.get(), position, state);
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new BoilerguiMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
